...
const formSchema = z.object({
  // Age verification
  isOver18: z.literal('yes', {
    errorMap: () => ({ message: "You must be at least 18 years old to register" }),
  }),
...
const FormData = formSchema._type; // Only if you still want to infer type-like data
...
export default function PatientRegistrationForm() {
  ...
  const handleNext = async () => {
    ...
    if (currentSegment === 0) {
      isValid = watch('isOver18') === 'yes';
      if (!isValid) {
        setShowAgeAlert(true);
        return;
      }
    } else {
      // Validate fields in current segment
      const segmentFields = getSegmentFields(segments[currentSegment].id);
      isValid = await trigger(segmentFields);
    }
  ...
  const getSegmentFields = (segmentId) => {
    switch (segmentId) {
      ...
      case 'address': return ['address1', 'address2', 'city', 'state', 'zip'];
          ...
          <div className="fixed bottom-0 left-0 right-0 py-4 ">
            <div className="container mx-auto max-w-6xl px-6">
              <div className="flex justify-between">
                ...
                {currentSegment > 0 ? (
                  <Button variant="outline" onClick={handlePrevious} type="button" className="bg-secondary text-white hover:text-white hover:bg-secondary rounded-2xl">
                    Previous
                  </Button>
                ) : (
                  <div></div>
                ...
                {currentSegment < segments.length - 1 ? (
                  <Button onClick={handleNext} type="button" className="bg-secondary text-white hover:bg-secondary rounded-2xl">
                    Continue
                  </Button>
                ) : (
                  <Button type="submit" disabled={!isValid} className="bg-green-400 text-white hover:bg-green-500 rounded-2xl">
                    Submit
                  </Button>
                )}
