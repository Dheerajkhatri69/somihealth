...
// POST - Create new skinhair refill questionnaire
export async function POST(req) {
    ...
    try {
        ...
        const count = await SkinHairRefillQuestionnaire.countDocuments({ authid: authIdRegex });
        ...
        await SkinHairRefillQuestionnaire.create(submissionData);

        // Update abandonment record to "completed" state if userSessionId exists
        if (userSessionId) {
            try {
                await fetch(`${req.headers.get('origin')}/api/skinhair-refill-abandonment`, {
                    method: 'POST',
                    headers: {
        ...
        return NextResponse.json({
            message: "SkinHair refill questionnaire submitted successfully.",
            authid: newAuthId,
            success: true
        }, { status: 201 });
    } catch (error) {
        console.error("Error submitting skinhair refill questionnaire: ", error);
...
// GET - Fetch all skinhair refill questionnaires
export async function GET(req) {
    try {
        ...
        if (authid) {
            ...
            questionnaires = await SkinHairRefillQuestionnaire.find({ authid });
        } else {
            // Fetch all, sorted by latest
            questionnaires = await SkinHairRefillQuestionnaire.find().sort({ createdAt: -1 });
        }
        ...
        console.error("Error fetching skinhair refill questionnaires: ", error);
        return NextResponse.json({
            success: false,
...
export async function DELETE(req) {
    try {
        ...
        result = await SkinHairRefillQuestionnaire.deleteMany(query);
        ...
        console.error("Error deleting skinhair refill questionnaires: ", error);
        return NextResponse.json({
            success: false,
...
// PUT - Update skinhair refill questionnaire
export async function PUT(req) {
    try {
        ...
        if (authid) {
            updatedQuestionnaire = await SkinHairRefillQuestionnaire.findOneAndUpdate(
                { authid: authid },
            ...
            updatedQuestionnaire = await SkinHairRefillQuestionnaire.findByIdAndUpdate(
                id,
                { ...updates, updatedAt: new Date() },
                { new: true, runValidators: true }
            );
        ...
        console.error("Error updating skinhair refill questionnaire: ", error);
        return NextResponse.json({
            success: false,
