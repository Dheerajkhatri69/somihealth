...
export async function GET() {
    try {
        await connectDB();
        const data = await SkinHairQuestionnaire.find().sort({ createdAt: -1 });
...
export async function POST(request) {
    try {
        ...
        const newDoc = new SkinHairQuestionnaire(body);
...
export async function DELETE(request) {
    try {
        ...
        const result = await SkinHairQuestionnaire.deleteMany({ authid: { $in: authids } });
