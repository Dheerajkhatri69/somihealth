...
export async function GET() {
    try {
        await connectDB();
        const data = await LongevityQuestionnaire.find().sort({ createdAt: -1 });
...
export async function POST(request) {
    try {
        ...
        const newDoc = new LongevityQuestionnaire(body);
...
export async function DELETE(request) {
    try {
        ...
        const result = await LongevityQuestionnaire.deleteMany({ authid: { $in: authids } });
