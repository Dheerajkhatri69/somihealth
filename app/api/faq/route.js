...
export async function GET() {
  try {
    ...
    const faqData = await FAQ.findOne({ isActive: true });
...
// POST - Create or update FAQ data
export async function POST(request) {
  try {
    ...
    // Deactivate any existing active FAQ
    await FAQ.updateMany({ isActive: true }, { isActive: false });
    
    // Create new FAQ with the provided data
    ...
    return NextResponse.json(
      { error: 'Failed to save FAQ data' },
      { status: 500 }
    );
  }
}

// PUT - Update existing FAQ data
export async function PUT(request) {
  try {
    ...
    // Find and update the active FAQ
    ...
    return NextResponse.json(
      { error: 'Failed to update FAQ data' },
      { status: 500 }
    );
  }
}
