...
export async function POST(request) {
    ...
    try {
        ...
        const result = await EDRefillAbandonment.findOneAndUpdate(
            { userSessionId },
            { $set: updateFields },
...
export async function GET() {
    await connectDB();
    try {
        const result = await EDRefillAbandonment.find().sort({ updatedAt: -1 });
        return NextResponse.json({ success: true, result });
    } catch (err) {
        return NextResponse.json({ success: false, message: err.message }, { status: 500 });
    }
...
export async function DELETE(request) {
    ...
    try {
        ...
        const deleted = await EDRefillAbandonment.findOneAndDelete({ userSessionId });
