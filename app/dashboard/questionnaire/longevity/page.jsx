...
export default function Dashboard() {
    ...
    const [filters, setFilters] = useState({
        search: "",
        dateRange: "all",
        treatmentChoose: "all", // 🔁 was glp1Preference
        sex: "all",
        heardAbout: "all",
    });
    ...
    const fetchData = async () => {
        try {
            ...
            if (result.success) {
                ...
                // 🔥 Show ONLY forms where questionnaire = true
                const activeForms = result.result.filter(
                    (item) => item.questionnaire === true
                );

                setData(activeForms);

                // NEW unseen count — only count active forms
                const unseenItems = activeForms.filter((item) => !item.seen);
                ...
                if (unseenItems.length > 0) {
                    ...
                    const markAsSeenResponse = await fetch(
                        "/api/longevity-questionnaire/mark-as-seen",
                        {
                            method: "POST",
    ...
    const filteredData = data.filter((item) => {
        ...
        // 🔁 treatmentChoose filter instead of glp1Preference
        const treatmentMatch =
            filters.treatmentChoose === "all" ||
            item.treatmentChoose === filters.treatmentChoose;

        const sexMatch = filters.sex === "all" || item.sex === filters.sex;

        // 🔁 use createdAt (from timestamps) instead of createTimeDate
        ...
        const dateMatch =
            ...
            (filters.dateRange === "week" && createdAt && diffInDays <= 7) ||
            (filters.dateRange === "month" && createdAt && diffInDays <= 30);
        ...
        return searchMatch && treatmentMatch && sexMatch && dateMatch && heardMatch;
    });
    ...
    return (
        <div className="overflow-x-auto px-4">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                {/* Treatment filter (was GLP-1 Preference) */}
                <Select
                    value={filters.treatmentChoose}
                    onValueChange={(value) =>
                        setFilters((prev) => ({ ...prev, treatmentChoose: value }))
                    }
                >
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    <DropdownMenuItem asChild>
                                                        <Link href={`/dashboard/questionnaire/longevity/${item.authid}`}>
                                                            Open
                                                        </Link>
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem asChild>
