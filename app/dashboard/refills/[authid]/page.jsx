...
export default function FollowUpForm({ params }) {
    ...
    useEffect(() => {
        const handleSearch = async () => {
            const baseAuthId = params.authid?.split('-')[0];
            if (!baseAuthId) return;
            ...
            try {
                ...
                const patientResponse = await fetch(`/api/patients/${baseAuthId}`);
                ...
                const countResponse = await fetch(`/api/followup/count/${baseAuthId}`);
                ...
                const newAuthId = followUpCount > 0 ? `${baseAuthId}-${followUpCount}` : baseAuthId;
                ...
                setFormData(prev => ({
                    ...
                    authid: newAuthId,
                    firstName: prev.firstName || patientData.patient.firstName || '',
                    lastName: prev.lastName || patientData.patient.lastName || '',
                    dob: patientData.patient.dob || '',
                    height: patientData.patient.height || '',
                    email: prev.email || patientData.patient.email || '',
                    sex: patientData.patient.sex || '',
                    weight: patientData.patient.weight || '',
                    phone: prev.phone || patientData.patient.phone || '',
                    ...
                    approvalStatus: prev.approvalStatus || patientData.patient.approvalStatus || '',
                    ...
                    providerComments: prev.providerComments || patientData.patient.providerComments || '',
            ...
            try {
                ...
                if (data.success) {
                    ...
                    setFormData(prev => ({
                        ...
                        glp1ApprovalLast6Months: data.result.glp1Approved,
                        currentWeight: data.result.currentWeight,
                        currentGlp1Medication: data.result.glp1Preference,
                        anySideEffects: data.result.sideEffects,
                        listSideEffects: data.result.sideEffectsDetail,
                        happyWithMedication: data.result.happyWithMed,
                        switchMedication:
                            (data.result.dosageDecision === "Change to Semaglutide" ||
                                data.result.dosageDecision === "Change to Tirzepatide")
                                ? data.result.dosageDecision
                                : "",

                        continueDosage:
                            data.result.dosageDecision === "keep the same dosage"
                                ? data.result.dosageDecision
                                : "",

                        increaseDosage:
                            (data.result.dosageDecision === "Increase my current dose" ||
                                data.result.dosageDecision === "Decrease my current dose" ||
                                data.result.dosageDecision === "increase my dosage")
                                ? data.result.dosageDecision
                                : "",
    ...
    const handleSubmit = async (e) => {
        ...
        if (!refillId) {
            ...
            setMessage("Could not find the original refill record to delete.");
            setIsSuccess(false);
        ...
        try {
            ...
            const deleteResponse = await fetch('/api/refills', {
                ...
                body: JSON.stringify({ ids: [refillId] }),
            });
