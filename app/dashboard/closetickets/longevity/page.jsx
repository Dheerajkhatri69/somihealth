...
const Page = () => {
    ...
    useEffect(() => {
        const fetchPatients = async () => {
            setLoading(true);
            try {
                const res = await fetch('/api/longevity-questionnaire');
    ...
    const handleRestore = async (authid) => {
        try {
            const response = await fetch(`/api/longevity-questionnaire/${authid}`, {
                method: 'PUT',
                    ...
                    {currentRows.map((patient) => (
                        <TableRow key={patient.authid}>
                            ...
                            <TableCell className="bg-white flex flex-wrap gap-2">
                                <Link href={`/dashboard/questionnaire/longevity/${patient.authid}`}>
