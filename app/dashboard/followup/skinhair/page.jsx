...
export default function FollowUp() {
    ...
    useEffect(() => {
        const fetchPatients = async () => {
            try {
                ...
                const patientsRes = await fetch("/api/skinhair-refill-questionnaire");
                ...
                let activePatients = (patientsData.result || []).filter(p => p.closetickets === false && p.questionnaire === false);
    ...
    const filteredPatients = patients.filter(patient => {
        ...
        const medicineMatch = medicineFilter === 'all' || patient.currentTreatment === medicineFilter;
        ...
        return emailOrNameMatch && pIdMatch && genderMatch && dobMatch && cityMatch &&
            medicineMatch && doseUnitMatch && approvalMatch && createDateMatch && clinicianMatch && createDateRangeMatch && followUpMatch && refillReminderMatch;
    });
    ...
    async function handleResolveFollowUp(patient) {
        ...
        try {
            const response = await fetch(`/api/skinhair-refill-questionnaire/${patient.authid}`, {
                method: "PUT",
                    ...
                    <Table className="min-w-full">
                        <TableHeader>
                            <TableRow>
                                ...
                                {/* Medical Information */}
                                <TableHead className="whitespace-nowrap">Current Treatment</TableHead>
                                ...
                                {/* DOse */}
                                <TableHead className="whitespace-nowrap">Dose</TableHead>
                        ...
                        <TableBody>
                            {currentRows.map((patient) => (
                                <TableRow key={patient.authid} >
                                    ...
                                    <TableCell>{patient.currentTreatment}</TableCell>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-52">
                                                ...
                                                {(userType === 'A' || userType === 'C') && (
                                                    ...
                                                    >
                                                        <Link href={`/dashboard/refills/skinhair/${patient.authid}`}>Open</Link>
                                                    </DropdownMenuItem>
                                                )}
