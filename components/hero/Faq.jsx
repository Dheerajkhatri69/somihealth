...
export default function FaqPro() {
  ...
  function Answer({ isOpen, id, labelledBy, children }) {
    ...
    useLayoutEffect(() => {
      ...
      // measure only on toggle/content change
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="grid items-start gap-10 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            {/* (Optional) remove Expand/Collapse since single-open */}
                  ...
                  return (
                    <li key={idx} className="group">
                      <button
                        ...
                        onClick={() => toggle(idx)}
                        className={`flex w-full items-start gap-4 px-5 py-4 text-left transition-colors sm:px-6 ${
                          isOpen ? "bg-secondary/3" : "hover:bg-gray-50"
                        }`}
                      >
...
function PlusMinusIcon({ open }) {
  return (
    <span
      className={`relative mt-1 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full ring-1 ring-gray-300 transition-colors ${
        open ? "bg-secondary text-white ring-secondary/60" : "bg-white text-gray-700"
      }`}
      aria-hidden="true"
    >
      <span
        className={`absolute h-3.5 w-0.5 rounded-sm transition-opacity ${
